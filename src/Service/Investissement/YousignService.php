...
class YousignService
{
    ...
    public function getStatus(string $signatureRequestId): array
    {
        try {
            $response = $this->httpClient->request(
                ...
                ['headers' => $this->jsonHeaders(), 'timeout' => 10.0]
            );
            return $response->toArray();
        } catch (TransportExceptionInterface $e) {
            throw new \RuntimeException(
                'Yousign inaccessible depuis ce réseau (timeout/connexion refusée).',
                0,
                $e
            );
        }
    ...
    public function downloadSignedDocument(Deal $deal): ?string
    {
        ...
        try {
            $response = $this->httpClient->request(
                ...
                ['headers' => ['Authorization' => 'Bearer ' . $this->apiKey], 'timeout' => 30.0]
            );

            // Force response buffering — throws on HTTP 4xx/5xx
    ...
    /** POST JSON and return decoded array; throws on 4xx/5xx or network error. */
    private function apiPost(string $path, array $body): array
    {
        try {
            ...
            throw new \RuntimeException(
                'Yousign inaccessible depuis ce réseau (timeout/connexion refusée). ' .
                'Vérifiez votre connexion internet ou utilisez un hotspot.',
                0,
                $e
            );
        }
    ...
    /** Upload a PDF via multipart/form-data; throws on 4xx/5xx or network error. */
