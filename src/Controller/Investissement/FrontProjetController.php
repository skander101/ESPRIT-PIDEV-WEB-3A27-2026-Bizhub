...
class FrontProjetController extends AbstractController
{
    ...
    {
        /** @var \App\Entity\UsersAvis\User $user */
        $user = $this->getUser();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $this->addFlash('error', 'Une erreur est survenue lors de la création du projet. Veuillez réessayer.');
            }
        }
    ...
    public function show(int $id): Response
    {
        ...
        // Deal le plus avancé pour le bouton "Télécharger le contrat PDF" (vue startup)
        // has_dompdf_contract = true dès que le contrat Dompdf a été généré (status >= paid)
        // => indépendant du PDF Yousign qui peut remplacer contract_pdf_path après signature
        $latestDeal       = null;
        $hasDompdfContract = false;
        $dompdfStatuses   = ['paid', 'pending_signature', 'signed', 'completed'];
        if ($isOwner) {
            foreach ($dealByNegMap as $deal) {
                if (!$deal) {
                    continue;
                }
                if (in_array($deal->getStatus(), $dompdfStatuses, true)) {
                    // Priorité au deal le plus avancé (signé > en attente de signature > payé)
                    if (!$latestDeal || !$hasDompdfContract) {
                        $latestDeal        = $deal;
                        $hasDompdfContract = true;
                    }
                } elseif (!$latestDeal) {
                    // Deal en pending_payment : on le garde comme fallback pour le message "indisponible"
                    $latestDeal = $deal;
                }
            }
        }
        ...
        return $this->render('front/projet/show.html.twig', [
            ...
            'has_dompdf_contract' => $hasDompdfContract,
        ]);
    }
