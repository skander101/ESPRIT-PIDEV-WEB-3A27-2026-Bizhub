...
class DealController extends AbstractController
{
    ...
    public function yousignSend(
        ...
        try {
            ...
            // ── Fallback 1 : email classique avec lien token ──────────────────
            try {
                $this->signatureEmailService->sendSignatureEmail($deal, $buyer);
                $this->addFlash('warning',
                    'Yousign indisponible depuis ce réseau. ' .
                    'Un lien de signature classique a été envoyé à ' . $buyer->getEmail() . ' (valide 48h).'
                );

            } catch (\Exception $mailEx) {
                // ── Fallback 2 : lien direct affiché dans la page (mode démo) ─
                // sendSignatureEmail() persiste le token AVANT l'envoi email,
                // donc $deal->getSignature_token() est disponible même si le mailer a planté.
                $token = $deal->getSignature_token();
                if ($token) {
                    $signUrl = $this->generateUrl('app_deal_sign_token', [
                        'id'    => $deal->getDeal_id(),
                        'token' => $token,
                    ], UrlGeneratorInterface::ABSOLUTE_URL);
                    $this->addFlash('warning',
                        'Yousign et email indisponibles (réseau restreint). ' .
                        'Lien de signature directe pour la démo : ' . $signUrl
                    );
                } else {
                    $this->addFlash('error',
                        'Yousign et email sont inaccessibles depuis ce réseau. ' .
                        'Connectez-vous via hotspot pour continuer.'
                    );
                }
            }
        }
    ...
    public function yousignSync(
        ...
        try {
            ...
            if ($done) {
                ...
                if ($signedPdfOk) {
                    $this->addFlash('success', '✅ Signature confirmée et contrat signé téléchargé depuis Yousign. Vous pouvez maintenant télécharger le PDF final.');
                } else {
                    $this->addFlash('success', '✅ Signature Yousign confirmée ! Le contrat est disponible au téléchargement (PDF initial conservé — le PDF signé sera récupéré lors du prochain accès réseau).');
                }
            } else {
            ...
            $msg = $e->getMessage();
            if (str_contains($msg, 'inaccessible') || str_contains($msg, 'timeout') || str_contains($msg, 'connect')) {
                $this->addFlash('warning',
                    'Yousign est inaccessible depuis ce réseau (timeout). ' .
                    'La signature Yousign ne peut pas être synchronisée pour l\'instant. ' .
                    'Vous pouvez utiliser le lien de signature classique via « Renvoyer l\'email ».'
                );
            } else {
                $this->addFlash('error', 'Erreur Yousign : ' . $msg);
            }
        }
    ...
    // ────────────────────────────────────────────────────────────────────────
    // GÉNÉRER ET TÉLÉCHARGER LE CONTRAT PDF VIA DOMPDF (on-the-fly)
    // ────────────────────────────────────────────────────────────────────────

    #[Route('/{id}/dompdf', name: 'app_deal_dompdf', methods: ['GET'], requirements: ['id' => '\d+'])]
    public function downloadPdf(
        #[MapEntity(mapping: ['id' => 'deal_id'])]
        Deal $deal,
        \App\Service\Investissement\DompdfService $dompdfService
    ): Response {
        $user = $this->getUser();
        if (!$user) {
            return $this->redirectToRoute('app_login');
        ...
        $buyer       = $this->em->getRepository(User::class)->find($deal->getBuyer_id());
        $seller      = $this->em->getRepository(User::class)->find($deal->getSeller_id());
        $project     = $this->em->getRepository(Project::class)->find($deal->getProject_id());

        $pdf = $dompdfService->generatePdf('pdf/contrat.html.twig', [
            'deal'        => $deal,
            'negotiation' => $negotiation,
            'project'     => $project,
            'buyer'       => $buyer,
        ...
        return new Response($pdf, 200, [
            ...
            'Content-Disposition' => 'attachment; filename="contrat-' . $deal->getDeal_id() . '.pdf"',
        ]);
    }
    ...
    #[Route('/{id}/certificat', name: 'app_deal_certificat', methods: ['GET'], requirements: ['id' => '\d+'])]
    public function certificat(
        ...
        $allowedStatuses = [Deal::STATUS_SIGNED, Deal::STATUS_COMPLETED];
        if (!in_array($deal->getStatus(), $allowedStatuses, true)) {
            $this->addFlash('error', 'Le certificat n\'est disponible qu\'après signature du contrat.');
            return $this->redirectToRoute('app_deal_show', ['id' => $deal->getDeal_id()]);
        }
        ...
        if (!$buyer || !$seller) {
            $this->addFlash('error', 'Données du deal incomplètes.');
            return $this->redirectToRoute('app_deal_show', ['id' => $deal->getDeal_id()]);
        }

        try {
            $certPath = $this->pdfService->generateCertificate($deal, $buyer, $seller);
            $absPath  = $this->getParameter('kernel.project_dir') . '/public' . $certPath;

            $response = new BinaryFileResponse($absPath);
            $response->setContentDisposition(
                ResponseHeaderBag::DISPOSITION_ATTACHMENT,
                sprintf('certificat-signature-deal-%d.pdf', $deal->getDeal_id())
            );
            $response->headers->set('Content-Type', 'application/pdf');
            ...
            $this->addFlash('error', 'Impossible de générer le certificat : ' . $e->getMessage());
            return $this->redirectToRoute('app_deal_show', ['id' => $deal->getDeal_id()]);
        }
    }
