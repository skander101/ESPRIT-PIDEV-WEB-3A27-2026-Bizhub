...
// Database configuration - LAMPP MySQL
$dbHost = 'localhost';
$dbName = 'BizHub';
$dbUser = 'root';
$dbPass = ''; // LAMPP default is empty password
$dbPort = 3306;
...
// Try different connection methods for LAMPP
$pdo = null;
$connectionErrors = [];

// Method 1: LAMPP Unix socket
try {
    echo "Attempting LAMPP Unix socket connection...\n";
    $pdo = new PDO("mysql:unix_socket=/opt/lampp/var/mysql/mysql.sock;dbname=$dbName", $dbUser, $dbPass);
    $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
    echo "✓ Connected to LAMPP MySQL via Unix socket successfully!\n";
} catch (PDOException $e) {
    $connectionErrors[] = "LAMPP socket: " . $e->getMessage();

    // Method 2: TCP connection to localhost
    try {
        echo "Attempting TCP connection to LAMPP MySQL...\n";
        $pdo = new PDO("mysql:host=$dbHost;port=$dbPort;dbname=$dbName", $dbUser, $dbPass);
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        echo "✓ Connected to LAMPP MySQL via TCP successfully!\n";
    } catch (PDOException $e2) {
        $connectionErrors[] = "LAMPP TCP: " . $e2->getMessage();

        echo "\n❌ Cannot connect to LAMPP MySQL!\n\n";
        echo "Errors encountered:\n";
        foreach ($connectionErrors as $error) {
            echo "  - $error\n";
        }
        echo "\n⚠️  LAMPP MySQL is not running or not accessible.\n\n";
        echo "To start LAMPP, run:\n";
        echo "  sudo /opt/lampp/lampp start\n\n";
        echo "To check LAMPP status:\n";
        echo "  sudo /opt/lampp/lampp status\n\n";
        echo "Make sure the 'BizHub' database exists in LAMPP:\n";
        echo "  /opt/lampp/bin/mysql -u root -e \"CREATE DATABASE IF NOT EXISTS BizHub;\"\n";
        die();
    }
}
