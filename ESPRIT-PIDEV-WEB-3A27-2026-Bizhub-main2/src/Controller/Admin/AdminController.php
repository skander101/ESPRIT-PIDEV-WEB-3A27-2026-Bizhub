...
class AdminController extends AbstractController
{
    ...
    public function userIndex(Request $request): Response
    {
        ...
        $users = $this->userRepository->findBySearchAndSort(
            $search !== '' ? $search : null,
            $sort,
            $direction
        );
    ...
    public function userDelete(Request $request, User $user): Response
    {
        ...
        if (!$this->isCsrfTokenValid('delete_user_' . $user->getUser_id(), $token)) {
    ...
    #[Route('/avis/{id}/remove', name: 'app_admin_avis_remove', methods: ['POST'], requirements: ['id' => '\d+'])]
    public function avisRemove(Request $request, Avis $avi): Response
    {
        $token = $request->request->get('_token');
        if (!$this->isCsrfTokenValid('remove_avis', $token)) {
            $this->addFlash('error', 'Invalid CSRF token.');
    ...
    #[Route('/avis/{id}/restore', name: 'app_admin_avis_restore', methods: ['POST'], requirements: ['id' => '\d+'])]
    public function avisRestore(Request $request, Avis $avi): Response
    {
        $token = $request->request->get('_token');
        if (!$this->isCsrfTokenValid('restore_avis', $token)) {
            $this->addFlash('error', 'Invalid CSRF token.');
            return $this->redirectToRoute('app_admin_avis_index');
        }

        $avi->setIsRemoved(false);
        $this->entityManager->flush();
        $this->addFlash('success', 'Review restored successfully!');
