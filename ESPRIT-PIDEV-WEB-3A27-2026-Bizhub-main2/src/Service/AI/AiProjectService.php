 ...
 * Accepte en option l'entité Project pour contextualiser la reformulation
 * avec les champs métier enrichis (problème, solution, marché, etc.).
...
class AiProjectService
{
     ...
     * Si $project est fourni, utilise ses champs métier pour contextualiser la reformulation.
     *
    ...
    public function improveDescription(string $description, ?Project $project = null): string
    {
        ...
        try {
            ...
            // Quota dépassé ou clé invalide → fallback local silencieux
    ...
    /**
     * Build the user message, optionally enriched with project context fields.
     */
    private function buildUserMessage(string $description, ?Project $project): string
    {
        ...
        if ($project !== null) {
            ...
            if ($opt($project->getProblemDescription()))   $context[] = 'Problème résolu : ' . $opt($project->getProblemDescription());
            if ($opt($project->getSolutionDescription()))  $context[] = 'Solution : ' . $opt($project->getSolutionDescription());
            if ($opt($project->getTargetAudience()))       $context[] = 'Public cible : ' . $opt($project->getTargetAudience());
            ...
            if ($opt($project->getCompetitiveAdvantage())) $context[] = 'Avantage concurrentiel : ' . $opt($project->getCompetitiveAdvantage());
        }
    ...
    /**
     * Amélioration locale minimale utilisée quand l'API est indisponible.
     * Capitalise la première lettre et s'assure que le texte se termine bien.
     */
